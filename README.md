...
**Example PNG** — a heatmap of the full probability distribution with percentile curves overlaid:

The PNG shows period (log₁₀ s) on the x-axis, power (dB) on the y-axis, and probability as a color mesh. Percentile lines (p5, p50, p95, etc.) are drawn on top so you can see the distribution shape at every period.
